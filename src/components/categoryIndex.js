...
const CategoryIndex = () => {
  ...
  return (
    <div className="flex justify-center flex-wrap px-4 gap-2 w-full relative ">
      <Link to="/" className="text-4xl text-white absolute left-16 py-2">Go Back Home</Link>
      {jobs.length > 0 ? (
        check.length !== 0 ? (
          check.map((x) => {
            const {
            ...
            return (
              <div
                ...
                key={id}
              >
                <div className="block p-6 rounded-lg shadow-lg bg-[#ec4c8b] w-64 lg:8/12 grow">
                  ...
                  <h2 className="text-white text-base leading-tight font-semibold mb-2">
                    {title}
                  </h2>
                  ...
                  <a
                    href={redirect_url}
                    target="_blank"
                    className="bg-gray-100 text-blue-600 px-4 py-2 rounded-md mt-3"
                    rel="noreferrer"
                  >
                    Apply
                  </a>
